...
class build_ext_with_cmake(build_ext):
    ...
    def run(self):
        ...
        hddm_dir = os.path.join(cwd, "gluex", "hddm_r")
        old_path = self.get_ext_fullpath("gluex.hddm_r")
        new_name = os.path.basename(old_path).replace("hddm_r", "__init__", 1)
        new_path = os.path.join(os.path.dirname(old_path), "hddm_r", new_name)
        ...
        keep = {"__init__.py", "pyxrootd", "hddm_r"}
...
if "win" in sysconfig.get_platform():
    extension_include_dirs = [os.path.join("gluex", "hddm_r"),
